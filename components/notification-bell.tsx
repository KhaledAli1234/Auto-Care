...
function typeIcon(type: ApiNotification['type']): keyof typeof Ionicons.glyphMap {
  switch (type) {
    ...
    case 'reply':        return 'return-down-forward-outline';
...
function typeColor(type: ApiNotification['type']): string {
  switch (type) {
    ...
    case 'mention':      return COLORS.warning;
    case 'system_alert': return COLORS.warning;
...
/* ════════════════════════════════════════
   BELL BUTTON (drop into any header)
════════════════════════════════════════ */
export function NotificationBell({ iconSize = 25, color = COLORS.text }: { iconSize?: number; color?: string }) {
...
function NotificationItem({ notification: n }: { notification: ApiNotification }) {
...
const styles = StyleSheet.create({
  bellWrap: { width: 40, height: 40, borderRadius: 20, borderWidth: 1, borderColor: 'rgba(255,255,255,0.07)', backgroundColor: '#172b44', alignItems: 'center', justifyContent: 'center' },
  ...
  sheet:    { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: COLORS.surface, borderTopLeftRadius: 28, borderTopRightRadius: 28, paddingHorizontal: 20, paddingTop: 12, maxHeight: '80%', borderWidth: 1, borderColor: COLORS.border },
  handle:   { width: 40, height: 4, backgroundColor: COLORS.mutedDark, borderRadius: 2, alignSelf: 'center', marginBottom: 16, opacity: 0.5 },

  sheetHeader:  { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
  sheetTitle:   { color: COLORS.text, fontSize: 20, fontWeight: '800' },
  markReadBtn:  { backgroundColor: 'rgba(50,104,247,0.12)', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 6 },
  ...
  notifItem:       { flexDirection: 'row', gap: 12, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: COLORS.divider },
  notifItemUnread: { backgroundColor: 'rgba(50,104,247,0.05)', marginHorizontal: -20, paddingHorizontal: 20 },
  ...
  notifSender:     { color: COLORS.mutedDark, fontSize: 12, marginTop: 2 },
  notifTime:       { color: COLORS.mutedDark, fontSize: 11, marginTop: 4 },
