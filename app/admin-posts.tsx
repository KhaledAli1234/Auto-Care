...
export default function AdminPostsScreen() {
  ...
  // FIX: استبدل الـ 3 calls بـ call واحد للـ endpoint الجديد
  // GET /posts/admin/list?status=pending|approved|rejected
  ...
  const fetchPosts = useCallback(async (isRefresh = false) => {
    try {
      ...
      // Fetch all 3 statuses in parallel with the correct endpoint
  ...
  // FIX: الـ endpoints الصح هي PATCH /posts/:postId/approve و /reject
  // اللي هي موجودة في الباك بالفعل
          ...
          {actionPost?.status === 'pending' && (
            <>
              <Pressable
                ...
                onPress={() => handleAction(actionPost._id ?? actionPost.id!, 'approve')}
                disabled={processing}
              >
                {processing
              ...
              <Pressable
                ...
                onPress={() => handleAction(actionPost._id ?? actionPost.id!, 'reject')}
                disabled={processing}
              >
                {processing
...
/* ════════════════════════════════════════
   POST CARD
════════════════════════════════════════ */
function PostModerationCard({ post, onAction }: { post: AdminPost; onAction: () => void }) {
  const authorName  = resolveAuthor(post.createdBy);
  const statusColor =
    post.status === 'approved' ? COLORS.success :
    post.status === 'rejected' ? COLORS.danger  : COLORS.warning;
  const statusIcon =
    post.status === 'approved' ? 'checkmark-circle' :
    post.status === 'rejected' ? 'close-circle'     : 'time';
    ...
    <View style={styles.postCard}>
      ...
      <Pressable style={styles.reviewBtn} onPress={onAction}>
        ...
        <Text style={styles.reviewBtnText}>
          {post.status === 'pending' ? 'Review & Decide' : 'Change Status'}
        </Text>
...
const styles = StyleSheet.create({
  ...
  reviewBtn:     { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: 'rgba(50,104,247,0.08)', borderRadius: 12, paddingHorizontal: 14, paddingVertical: 10, borderWidth: 1, borderColor: 'rgba(50,104,247,0.2)' },
  ...
  rejectBtn:      { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, height: 52, borderRadius: 16, borderWidth: 1, borderColor: COLORS.danger, backgroundColor: 'rgba(239,68,68,0.08)', marginBottom: 10 },
